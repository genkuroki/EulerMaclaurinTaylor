...
7月28日(Ver.0.04, 21頁): 剰余項の逐次積分による表示
に関する\secref{sec:nextK=P+intK}を追加した.
7月28日(Ver.0.05, 23頁): 積分の近似計算に関するSimpsonの公式の
誤差の大きさの評価に関する\secref{sec:formal-Simpson}を追加した.
7月29日(Ver.0.06, 24頁): \secref{sec:formal-higher-Simpson}を追加した.
2018年6月23日(Ver.0.06a, 25頁): 微小な訂正.
7月6日(Ver.0.6b, 25頁): 微小な修正.
}}
\def\ABSTRACT{%
...
\subsection{剰余項を与える核函数の不定積分表示}
\label{sec:nextK=P+intK}
...
Simpsonの公式の区間 $[x_k,x_{k+1}]$ に対応する部分
\[
  2h\frac{f(x)+4f(x+h)+f(x+2h)}{6}
\]
の形に合わせて, $\R$ 上の確率測度 $P(dy)$ を
\[
  \int_\R \varphi(y)\,P(dy) = \frac{\varphi(0)+4\varphi(h)+\varphi(2h)}{6}
\]
と定める. そして函数 $f(x)$ に対して,
\[
  \int_\R g(x+y)\,P(dy) = \frac{1}{2h}\int_0^{2h} f(x+y)\,dy
\]
を満たす函数 $g(x)$ を見付けたい.
もしもこのような $g(x)$ が見付かれば, Simpsonの公式中の $f(x)$
を $g(x)$ で置き換えることによって,
誤差無しで $f(x)$ の定積分を計算できる.
だから, $g(x)$ と $f(x)$ の違いがどれだけの大きさであるかを
見積もることができれば, Simpsonの公式による定積分の数値計算の
誤差の大きさを評価できるだろう.

函数 $g(x)$ に関する上の条件は形式的に次のように書き直される:
\[
  \int_\R e^{yD}g(x)\,P(dy) = \frac{1}{2h}\int_0^{2h} e^{yD}f(x)\,dy,
  \qquad D=\frac{d}{dx}.
\]
両辺の積分を形式的に実行すると,
\[
  \frac{1+4e^{hD}+e^{2hD}}{6}g(x)
  = \frac{e^{2hD}-1}{2hD}f(x).
\]
これは形式的には
\[
  g(x) = \frac{6}{1+4e^{hD}+e^{2hD}}\frac{e^{2hD}-1}{2hD}f(x)
\]
と解ける.

ここで, 次のようになっていることに注意せよ:
\begin{align*}
  \frac{1+e^{X/2}+e^X}{6}
  &= 1 + \frac{X}{2} + \frac{X^{2}}{6} + \frac{X^{3}}{24}
  + \frac{5 X^{4}}{576} + \cdots,
  \\
  \frac{e^X-1}{X}
  &= 1 + \frac{X}{2} + \frac{X^{2}}{6} + \frac{X^{3}}{24}
  + \frac{X^{4}}{120} + \cdots
\end{align*}
これらは3次まで一致しており, 4次の係数もかなり近い:
\[
  \frac{1}{120}-\frac{5}{576}=-\frac{1}{2880}.
\]
ゆえに
\begin{align*}
  \frac{6}{1+e^{X/2}+e^X}\frac{e^X-1}{X}
  = 1 - \frac{X^4}{2880} + O(X^6)
\end{align*}
となることがわかる%
\footnote{このような計算が苦手な人は %
\href
{https://www.wolframalpha.com}
{\TT WolframAlpha.com}
などを使ってもよい.}.
左辺は偶函数なので, 右辺に奇数次の項は出て来ない%
\footnote{左辺が偶函数であることは分子分母を $e^{X/2}$ で割れば分かる.}.

ゆえに, $X=2hD$ を代入することによって,
\[
  g(x)
  =\left(1-\frac{(2hD)^4}{2880}+O(h^6)\right)f(x)
  = f(x)-\frac{h^4}{180}f^{(4)}(x) + O(h^6).
\]
したがって, Simpsonの公式の誤差の主要部分は,
$A_4=\sup_{x\in[a,b]}|f^{(4)}(x)|$ とおくとき,
\[
  \left|
    \sum_{k=0}^n
    2h\frac{h^4}{180}\frac{f^{(4)}(x_k)+4f^{(4)}(x_k+h)+f^{(4)}(x_k+2h)}{6}
  \right|
  \leqq
  2h\sum_{k=0}^{N-1}\frac{h^4}{180}A_4
  = \frac{(b-a)A_4}{180}h^4
\]
のように上から抑えられる.
Simpsonの公式による定積分の数値計算の誤差は,
$h$ が小さいとき, 大体 $(b-a)h^4A_4/180$ 以下になると考えられる.
...
\begin{remark}
  以上の結果より,
  公式($*$)では $4N$ 個の和で誤差の絶対値は大体
  \[
    \frac{2(b-a)}{945}A_6\left(\frac{b-a}{4N}\right)^6
    =\frac{b-a}{1935360}A_6\left(\frac{b-a}{N}\right)^6
  \]
  で上から抑えられる.

  \secref{sec:formal-Simpson}の結果より,
  Simpsonの公式では $2N$ 個の和で誤差が大体
  \[
    \frac{b-a}{180}A_4\left(\frac{b-a}{2N}\right)^4
    =\frac{b-a}{2880}A_4\left(\frac{b-a}{N}\right)^4
  \]
  で上から抑えられるのであった.
  Simpsonの公式でも $4N$ 個の和を取ることにすれば誤差は大体
  \[
    \frac{b-a}{180}A_4\left(\frac{b-a}{4N}\right)^4
    =\frac{b-a}{46080}A_4\left(\frac{b-a}{N}\right)^4
  \]
  で上から抑えられることになる.
  これで上の対応する式を割ると
  \[
    \frac{A_6}{42A_4}\left(\frac{b-a}{N}\right)^2.
  \]
  これが $1$ より小さくなる場合にはSimpsonの公式よりも公式($*$)で
  数値計算した方が良いと考えられる.
  \qed
\end{remark}
